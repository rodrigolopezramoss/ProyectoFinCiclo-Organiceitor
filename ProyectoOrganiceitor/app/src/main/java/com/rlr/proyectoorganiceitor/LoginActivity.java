...
public class LoginActivity extends AppCompatActivity {
    ...
    private void doLogin() {
        ...
        if (ema.isEmpty()||pas.isEmpty()){
            Snackbar s = Snackbar.make(btnEntrar,R.string.lab_errorNo, BaseTransientBottomBar.LENGTH_LONG);
            s.show();
        }else {
            fbauth.signInWithEmailAndPassword(ema,pas)
                    .addOnCompleteListener(task -> {
                        if (!task.isSuccessful()) {
                            Snackbar s = Snackbar.make(btnEntrar,R.string.lab_errorInco, BaseTransientBottomBar.LENGTH_LONG);
                            s.show();
                        }else {
